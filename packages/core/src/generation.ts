...
export const generateImage = async (
    ...
    elizaLogger.logColorful("Generating image with OpenAI!!!!!!!!!!!!!!");
    const modelSettings = getImageModelSettings(runtime.imageModelProvider);
    elizaLogger.logColorful("Model settings:", modelSettings);
    const model = modelSettings.name;
    elizaLogger.logColorful("Model:", model);
    elizaLogger.logColorful("Generating image with options:", {
    ...
    elizaLogger.logColorful("API Key:", apiKey);
    try {
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            ...
            elizaLogger.log("Generating image with OpenAI!!!!!!!!!!!!!!");
            ...
            elizaLogger.logColorful("generateImage Target size:", targetSize);
            const openaiApiKey = runtime.getSetting("OPENAI_API_KEY") as string;
            elizaLogger.logColorful("generateImage OpenAI API Key:", openaiApiKey);
            if (!openaiApiKey) {
            ...
            elizaLogger.logColorful('generateImage success: true');
            return { success: true, data: base64s };
        }
    } catch (error) {
        console.error(error);
