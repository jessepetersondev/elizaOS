...
const mintNFTAction: Action = {
    ...
    handler: async (
        ...
        try {
            elizaLogger.log("Mint NFT Action Composing state for message:", message);
            if (!state) {
                state = (await runtime.composeState(message)) as State;
            } else {
                state = await runtime.updateRecentMessageState(state);
            }
            elizaLogger.log("Mint NFT Action Composed state:", state);
            const context = composeContext({
                state,
                template: mintNFTTemplate,
            });
            elizaLogger.log("Mint NFT Action Composed context:", context);
            const chains = _SupportedChainList;
            elizaLogger.log("Mint NFT Action Supported chains:", chains);
            const supportedChains: (
                | (typeof chains)[number]
                | "solana"
                | null
                )[] = [...chains, "solana", null];
            elizaLogger.log("Mint NFT Action Supported chains:", supportedChains);
            ...
            elizaLogger.log("Mint NFT Action Context with chains:", contextWithChains);
            ...
            elizaLogger.log("Mint NFT Action Generate Object:", res);
            const content = res.object as {
                collectionAddress: string,
                chainName: (typeof supportedChains)[number];
            };
            elizaLogger.log("Mint NFT Action Generate Object:", content);

            if (!isMintNFTContent(content)) {
                elizaLogger.error("Mint NFT Action Invalid content for MINT_NFT action.");
                if (callback) {
                    callback({
            ...
            if (content?.chainName === "solana") {
                ...
                if (metadata) {
                    ...
                    elizaLogger.log("Mint NFT Action NFT Address:", nftRes);
