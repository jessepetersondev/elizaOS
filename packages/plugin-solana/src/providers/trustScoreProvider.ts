...
import { RealTradingService } from "./realTradingService.ts";
...
export class TrustScoreManager {
    ...
    private async hasOpenTrade(tokenAddress: string, recommenderId: string): Promise<boolean> {
        const existingTrade = await this.trustScoreDb.getLatestTradePerformance(
            tokenAddress,
            recommenderId,
            false // not simulation
        );
        return existingTrade && !existingTrade.sell_timeStamp;
    }
    ...
    private async updateTrustScores(
        ...
        try {
            const currentMetrics = await this.trustScoreDb.getRecommenderMetrics(recommenderId);
            if (!currentMetrics) {
                return;
            }

            const TRUST_ADJUSTMENT = 0.1;
            const newTrustScore = success
                ? Math.min(1, currentMetrics.trustScore + TRUST_ADJUSTMENT)
                : Math.max(0, currentMetrics.trustScore - TRUST_ADJUSTMENT);

            await this.trustScoreDb.updateRecommenderMetrics({
                ...currentMetrics,
                trustScore: newTrustScore,

                totalRecommendations: currentMetrics.totalRecommendations + 1,
                successfulRecs: success
                    ? currentMetrics.successfulRecs + 1
                    : currentMetrics.successfulRecs
            });
        } catch (error) {
            elizaLogger.error("Error updating trust scores:", error);
        }
    }
    async validateTrade(
        tokenAddress: string,
        amount: number,
        recommenderId: string | null = null
    ): Promise<boolean> {
        try {
            // Get trust score for token/recommender
            const trustScore = await this.trustScoreDb.getRecommenderMetrics(tokenAddress);

            // Minimum trust score threshold
            const MIN_TRUST_SCORE = 0.5;

            if (trustScore.trustScore < MIN_TRUST_SCORE) {
                elizaLogger.warn(`Trade rejected due to low trust score: ${trustScore}`);
                return false;
            }

            return true;
        } catch (error) {
            elizaLogger.error("Error validating trade:", error);
            return false;
        }
    }
    async executeSell(
        ...
        try {
            ...
            const isValid = await this.validateTrade(tokenAddress, amount, recommenderId);
            if (!isValid) {
                return false;
            }

            // Execute the trade
            const success = await this.realTradingService.executeSell(
                tokenAddress,
                amount,
                recommenderId
            );

            if (success && recommenderId) {
                // Update trust scores based on trade outcome
                await this.updateTrustScores(tokenAddress, recommenderId, success);
            }
    ...
    async createTradePerformance(
        ...
        // Check if trade with this signature already exists
        if (transactionSignature) {
            const existingTrade = await this.trustScoreDb.getTradeBySignature(transactionSignature);
            if (existingTrade) {
                elizaLogger.warn(`Trade already recorded for signature ${transactionSignature}`);
        ...
        this.trustScoreDb.addTradePerformance(creationData, false);
    ...
    async updateSellDetails(
        ...
        // Only update if trade exists and hasn't been sold
        if (!trade || trade.sell_timeStamp) {
            elizaLogger.warn(`No open trade found for ${tokenAddress} to update sell details`);
            return null;
        }
