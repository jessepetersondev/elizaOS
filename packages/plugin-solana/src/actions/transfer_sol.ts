...
import { executeWithAutoTrading } from "./swapUtils";
...
interface TransferSolContent extends Content {
    recipientAddress: string;
    amount: string | number;
}

export function isTransferSolContent(content: unknown): content is TransferSolContent {
    if (!content || typeof content !== "object") return false;

    const c = content as Partial<TransferSolContent>;
    elizaLogger.log("Content for SOL transfer", c);

    return !!(
        typeof c.recipientAddress === "string" &&
...
export const transferSol = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        return isTransferSolContent(message.content);
    },
    description: "Transfer SOL to a recipient address using the auto-trading system",
    handler: async (
