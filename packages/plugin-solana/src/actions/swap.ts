...
import { executeWithAutoTrading, getTokenDecimals } from "./swapUtils.ts";
...
export function isSwapContent(content: unknown): content is SwapContent {
    ...
    const c = content as Partial<SwapContent>;
    elizaLogger.log("Content for swap", c);

    return !!(
        typeof c.fromTokenAddress === "string" &&
        typeof c.toTokenAddress === "string" &&
...
export const swap = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        return isSwapContent(message.content);
    },
    description: "Swap tokens using the auto-trading system",
    handler: async (
        ...
        const result = await executeWithAutoTrading(runtime, async () => {
            ...
            if (!response.inputTokenCA && response.inputTokenSymbol) {
                elizaLogger.log(
                    `Attempting to resolve CA for input token symbol: ${response.inputTokenCA}`
                );
                if(response.inputTokenSymbol.length > 10){
