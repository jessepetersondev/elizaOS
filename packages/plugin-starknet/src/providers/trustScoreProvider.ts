...
export class TrustScoreManager {
    ...
    async createTradePerformance(
        ...
        // Only create if no existing trade with this signature
        if (transactionSignature) {
            const existing = await this.trustScoreDb.getTradeBySignature(transactionSignature);
            if (existing) {
                elizaLogger.warn(`Skipping duplicate trade signature: ${transactionSignature}`);
                return existing;
            }
        }
